...
(with-eval-after-load 'agent-shell
  ...
  (setopt agent-shell-anthropic-default-model-id "sonnet"
  ...
  ;; Claude の ACP 通知と Codex のローカル rollout から時間枠制限を表示する。
  (unless (advice-member-p #'my-agent-shell--capture-claude-rate-limit
                           'agent-shell--update-usage-from-notification)
    (advice-add 'agent-shell--update-usage-from-notification :before
                #'my-agent-shell--capture-claude-rate-limit))
  ;; 使用率が入るのは生の rate_limit_event の方なので、新しいセッションを作るとき
  ;; に転送を頼み、その通知を拾う。頼めるのは session/new の時だけ。
  (unless (advice-member-p #'my-agent-shell-request-claude-rate-limit-events
                           'agent-shell-anthropic-make-claude-code-config)
    (advice-add 'agent-shell-anthropic-make-claude-code-config :filter-return
                #'my-agent-shell-request-claude-rate-limit-events))
  (unless (advice-member-p #'my-agent-shell--capture-claude-raw-rate-limit

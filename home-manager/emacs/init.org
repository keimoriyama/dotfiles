...
;; 拡張群のコマンドは C-c A 配下にまとめる。
;; C-c a は org-agenda が使っているので upstream 推奨の C-c A を使う。
(defvar-keymap agent-shell-extras-map
  :doc "Keymap for agent-shell 拡張パッケージのコマンド."
  "m" #'agent-shell-manager-toggle
  "j" #'agent-shell-attention-jump
  "d" #'agent-shell-attention-dashboard)
(keymap-global-set "C-c A" agent-shell-extras-map)
#+end_src

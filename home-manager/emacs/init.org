...
** agent-shell
#+begin_src emacs-lisp
(keymap-global-set "C-c C-a" #'agent-shell)
#+end_src
...
** agent-shell-manager
agent-shell バッファ一覧を tabulated-list で表示し、状態確認と管理を行う。
...
(pretty-hydra-define hydra-agent-shell
  (:title "🤖 Agent Shell" :color blue :quit-key "q" :foreign-keys warn :separator "╌")
   ("Shell"
    ...
    "Manage"
    (("g" agent-shell-manager-toggle "manager")
     ("j" agent-shell-attention-jump "jump pending")
     ("d" agent-shell-attention-dashboard "dashboard")

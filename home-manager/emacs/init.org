...
(with-eval-after-load 'lsp-completion
  ;; config
  (setopt lsp-completion-enable t
...
(with-eval-after-load 'python-ts-mode
  ...
  ;; Work around Emacs 30.2 + libtree-sitter 0.26 rejecting predicates
  ;; emitted by the built-in python-ts-mode font-lock queries.
  (when (and (version= emacs-version "30.2")
             (fboundp 'treesit-library-abi-version)
             (>= (treesit-library-abi-version) 15))
    (setq python--treesit-settings
          (append
           (seq-remove (lambda (setting)
                         (memq (nth 2 setting) '(keyword builtin type)))
...
;; Work around `agent-shell--extract-tool-parameters' treating a patch STRING as
;; a sequence of chars (e.g. "*" => 42), which triggers `(car 42)` errors.
;;(with-eval-after-load 'agent-shell
;;  (when (fboundp 'agent-shell--extract-tool-parameters)
;;    (advice-add
;;     'agent-shell--extract-tool-parameters
;;     :around
;;     (lambda (orig tool-parameters)
;;       (if (or (null tool-parameters) (stringp tool-parameters))
;;           tool-parameters
;;         (funcall orig tool-parameters))))))
#+end_src

...
(add-hook 'literate-haskell-mode-hook #'eglot-ensure)
#+end_src
...
(defun my/skk-return ()
  "かなモードでの C-j を RET と同じ動作にする。"
  (interactive)
  ;; newline を直接呼ばないのは、org-mode や minibuffer など
  ;; モードごとの RET の定義をそのまま活かしたいため。
  (let ((command (or (keymap-lookup nil "RET") #'newline)))
    (setq this-command command)
    (call-interactively command)))

(defun my/skk-setup-return-key ()
  "かなモードの C-j に `my/skk-return' を割り当てる。"
  ;; skk-mode に入るたびに設定し直すのは、skk がローマ字ルールを
  ;; コンパイルする際に C-j へ skk-insert (確定) を割り当てるため。
  (keymap-set skk-j-mode-map "C-j" #'my/skk-return))

(with-eval-after-load 'skk
 (add-hook 'skk-mode-hook #'ddskk-posframe-mode)
 (add-hook 'skk-mode-hook #'my/skk-setup-return-key))
#+end_src
...
(require 'oj)
(setopt oj-home-dir
        (expand-file-name "~/ghq/github.com/keimoriyama/haskell_atcoder/"))
(setq oj-default-online-judge 'atcoder)
#+end_src
** atcoder-cli
atcoder-cli (=acc=) でコンテストの問題一覧とサンプルケースを取得し、
=oj-home-dir= 以下にコンテストディレクトリを作成してそこへ移動する。
#+begin_src emacs-lisp
(defun atcoder-new-contest (contest-id)
  "atcoder-cli で CONTEST-ID の情報を取得してディレクトリを作成し、そこへ移動する。"
  (interactive "sContest ID: ")
  (let* ((default-directory oj-home-dir)
         (contest-dir (expand-file-name contest-id oj-home-dir))
         (buf (get-buffer-create "*acc new*")))
    (with-current-buffer buf
      (erase-buffer))
    (unless (zerop (call-process "acc" nil buf t "new" contest-id "-c" "all"))
      (pop-to-buffer buf)
      (error "acc new %s に失敗しました" contest-id))
    (dired contest-dir)))
#+end_src
** arto
#+begin_src emacs-lisp

...
(with-eval-after-load 'org
  ...
  (setq org-agenda-start-on-weekday 3
        ...
        org-agenda-files (list org-prj-dir org-journal-dir
                               (format "%s/%s" org-directory org-roam-dailies-directory org-journal-dir))
...
(setq org-icalendar-use-deadline '(todu-due event-if-todo-not-done event-if-todo event-if-not-todo))

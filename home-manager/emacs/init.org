...
(with-eval-after-load 'lsp-mode
  ...
  ;; Disable LSP inline completion to prevent conflicts with copilot
  (setopt lsp-inline-completion-enable nil)
  
  ;; Disable file watchers to prevent file descriptor exhaustion
  (setopt lsp-enable-file-watchers nil)
  (setopt lsp-file-watch-threshold 20000)
  
  ;; Prevent automatic workspace folder registration
...
***** python
#+begin_src emacs-lisp
;; Configure flycheck to use ruff for linting
(with-eval-after-load 'flycheck
  (flycheck-define-checker python-ruff
    ...
    :standard-input t
    :error-patterns
    ((error line-start (file-name) ":" line ": [" (id (one-or-more (not (any "]")))) "] " (message) line-end)
     (warning line-start (file-name) ":" line ": [" (id (one-or-more (not (any "]")))) "] " (message) line-end))
    :modes (python-mode python-ts-mode))
  
  ;; Add ruff checker to python
...
;; Use ty for type checking and ruff for linting
;; Disable basedpyright and ruff-lsp (we'll use ruff via flycheck for linting only)
(defun my/python-lsp-setup ()
  "Setup LSP for Python with ty only, disabling pyright and ruff LSP servers."
  (setq-local lsp-disabled-clients '(ruff-lsp ruff ty-ls)))
...
***** nix
#+begin_src emacs-lisp
(defun my/nix-lsp-setup ()
  "Setup LSP for Nix, disabling nix-nil in favor of nixd."
  (setq-local lsp-disabled-clients '(nix-nil)))

(add-hook 'nix-mode-hook #'my/nix-lsp-setup)
...
(with-eval-after-load 'lsp-nix
  ...
  (let ((flake-path (concat "builtins.getFlake \""
    ...
    (setopt lsp-nix-nixd-nixos-options-expr
            (format "(%s).homeConfigurations.myHomeConfig.options" flake-path))
    (setopt lsp-nix-nixd-home-manager-options-expr
            (format "(%s).homeConfigurations.myHomeConfig.options.home-manager.users.type.getSubOptions []" flake-path))))
#+end_src
***** web-mode
#+begin_src emacs-lisp
...
;; Disable pet file notifications to prevent file descriptor exhaustion
(require 'pet)
(setq pet-use-file-notifications nil)
(add-hook 'python-ts-mode-hook (lambda ()
                              (pet-mode)))
...
(reformatter-define python-isort-format
  ...
  :lighter "isort"
  :group 'python-isort)
(add-hook 'python-ts-mode-hook #'python-isort-format-on-save-mode)

(reformatter-define python-ruff-format
  :program "ruff"
  :args `("format" "--stdin-filename" ,buffer-file-name))
(add-hook 'python-ts-mode-hook #'python-ruff-format-on-save-mode)
